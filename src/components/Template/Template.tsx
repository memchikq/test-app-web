...
const Template: FC<TemplateProps> = ({ templateData }) => {
  ...
  const refetchTemplateData = async () => {
    const data = await ApiService.templateApiService.getTemplate()
    setTemplates(data)
  }
  ...
  return (
    <div>
      ...
      {openModal && <CreateTemplateModal openModal={openModal} refetchTemplateData={refetchTemplateData} closeModal={closeModal} />}
    </div>
  )
}
